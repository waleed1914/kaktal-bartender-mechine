...
class Edit(customtkinter.CTkFrame):
    ...
    def __init__(self, master, controller):
        ...
        self.title = customtkinter.CTkLabel(master=self, text="Edit Profile", font=("helvetica", 40), bg_color="white", fg_color="white", text_color="#265BAA")
        self.title.grid(row=0, column=0, sticky="nsew", pady=50, padx=0, columnspan=10)

        # ROW 1

        # add image up1.png on a button to the frame
        ...
        # # add a label with grey background to the frame
        # self.label9 = customtkinter.CTkLabel(master=self, text="Save Profile", font=("helvetica", 35), bg_color="light blue", fg_color="light blue", text_color="black", width=70, height=60)
        ...
        self.save_button = customtkinter.CTkButton(master=self.button_container, text="SAVE", font=("helvetica", 35), bg_color="white", fg_color="light green", text_color="black", hover_color="#00b050", height=80, text_color_disabled="white", command=self.save)
        self.save_button.grid(row=6, column=0, sticky="ew", pady=20, padx=60)

        # add a rounded button for edit profile
        self.button = customtkinter.CTkButton(master=self.button_container, text="DELETE", font=("helvetica", 35), bg_color="white", fg_color="#e6b9b8", text_color="black", hover_color="#e6b9b8", height=80, command=self.delete_profile)
        self.button.grid(row=7, column=0, sticky="ew", pady=20, padx=60)

        # add a rounded button for BACK profile
        self.button = customtkinter.CTkButton(master=self.button_container, text="BACK", font=("helvetica", 35), bg_color="white", fg_color="light blue", text_color="black", hover_color="#65aaff", height=80, command=self.back)
        self.button.grid(row=8, column=0, sticky="ew", pady=20, padx=60)
    ...
    def start_app(self):
        self.controller.frames['home'].add_ui()
    ...
    def delete_profile(self):
        CustomMessageBox(self.master, message="Are you sure you want to delete this profile?", command=self.start_delete)
    ...
    def start_delete(self):
        ...
        del records[self.profile]
        with open('records.pickle', 'wb') as f:
            pickle.dump(records, f)
        self.controller.show_frame("home")

    def hide_save_button(self):
        self.save_button.configure(state=tk.DISABLED,fg_color="white")

    def show_save_button(self):
        self.save_button.configure(state=tk.ACTIVE, fg_color="light green")
    ...
    def load(self, profile):
        ...
        self.title.configure(text=f"Edit {self.records['name']}")
    ...
    def save(self):
        ...
        self.records['1'] = float(self.station1.cget('text'))
        self.records['2'] = float(self.station2.cget('text'))
        self.records['3'] = float(self.station3.cget('text'))
        self.records['4'] = float(self.station4.cget('text'))
        self.records['5'] = float(self.station5.cget('text'))
        self.records['6'] = float(self.station6.cget('text'))
        self.records['7'] = float(self.station7.cget('text'))
        self.records['8'] = float(self.station8.cget('text'))
        self.records['9'] = float(self.station9.cget('text'))
        self.records['10'] = float(self.station10.cget('text'))
        self.records['11'] = float(self.station11.cget('text'))
        self.records['12'] = float(self.station12.cget('text'))
        self.records['13'] = float(self.station13.cget('text'))


        print(self.records)
        with open('records.pickle', 'rb') as f:
            records = pickle.load(f)
        records[self.profile] = self.records
        print(records)
        with open('records.pickle', 'wb') as f:
            pickle.dump(records, f)
        self.start_app()
